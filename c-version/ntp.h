...
typedef void (*ntp_f)(FILE* stream, const char* a, const char* op, const char* b, const char a_value[A_VALUE_STRING],
...
//
inline void ntp_default_f(FILE* stream, const char* a, const char* op, const char* b, const char a_value[A_VALUE_STRING],
                          const char b_value[B_VALUE_STRING], time_t time, const char* file, const char* function, int line, const char* format,
                          ...) {
  // void* udata;
  va_list args;
...
extern inline void ntp_default_f(FILE* stream, const char* a, const char* op, const char* b, const char a_value[A_VALUE_STRING],
...
#define ntp_type_full_f(stream, prefix, suffix, T, fmt, a, op, b, f, format, ...)                                                               \
  do {                                                                                                                                          \
    ...
    if (!(ntp_tmp_a_ op ntp_tmp_b_)) {                                                                                                          \
      char a_value[A_VALUE_STRING];                                                                                                             \
      snprintf(a_value, sizeof a_value, prefix "%" fmt suffix, ntp_tmp_a_);                                                                     \
      char b_value[B_VALUE_STRING];                                                                                                             \
      snprintf(b_value, sizeof b_value, prefix "%" fmt suffix, ntp_tmp_b_);                                                                     \
      const time_t t = time(NULL);                                                                                                              \
      (f)(stream, #a, #op, #b, a_value, b_value, t, __FILE__, __func__, __LINE__, format, ##__VA_ARGS__);                                       \
    }                                                                                                                                           \
  } while (0)
//
...
#endif  // ntp_type_full
        //
#define ntp_ntp_f(stream, expr, f, ...)                                                                                                         \
  do {                                                                                                                                          \
    bool ntp_tmp_expr_ = (expr);                                                                                                                \
    if (!(ntp_tmp_expr_)) {                                                                                                                     \
      char expr_value[EXPR_VALUE_STRING];                                                                                                       \
      snprintf(expr_value, sizeof expr_value, "%s", ntp_tmp_expr_ ? "true" : "false");                                                          \
      const time_t t = time(NULL);                                                                                                              \
      (f)(stream, #expr, "", "", expr_value, "", t, __FILE__, __func__, __LINE__, #__VA_ARGS__);                                                \
    }                                                                                                                                           \
  } while (0)
//
#if !defined(ntp_ntp)
...
// #define ntp_string_equal(a, b, ...)           ntp_true((strcmp((a), (b)) == 0), __VA_ARGS__)
