...
extern char *opt_justify(char *s, int width, int indent, int initial,
                         char *prefix);
...
#define opt_mess_4(fmt, var1, var2, var3, var4)                                \
  printf(fmt, var1, var2, var3, var4)
...
#define opt_snprintf_3(buf, n, fmt, v1, v2, v3)                                \
  snprintf(buf, n, fmt, v1, v2, v3)
#define opt_snprintf_4(buf, n, fmt, v1, v2, v3, v4)                            \
  snprintf(buf, n, fmt, v1, v2, v3, v4)
...
#define opt_fatal_1(fmt, var1)                                                 \
  do {                                                                         \
    char gstr[OPT_ERRMAXSTRLEN];                                               \
    opt_snprintf_1(gstr, OPT_ERRMAXSTRLEN, fmt, var1);                         \
    opt_fatal(gstr);                                                           \
  } while (0)
#define opt_fatal_2(fmt, var1, var2)                                           \
  do {                                                                         \
    char gstr[OPT_ERRMAXSTRLEN];                                               \
    opt_snprintf_2(gstr, OPT_ERRMAXSTRLEN, fmt, var1, var2);                   \
    opt_fatal(gstr);                                                           \
  } while (0)
#define opt_fatal_3(fmt, var1, var2, var3)                                     \
  do {                                                                         \
    char gstr[OPT_ERRMAXSTRLEN];                                               \
    opt_snprintf_3(gstr, OPT_ERRMAXSTRLEN, fmt, var1, var2, var3);             \
    opt_fatal(gstr);                                                           \
...
char *opt_justify(char *s, int width, int indent, int initial, char *prefix) {
  ...
  bufSize = initial + width + 1 + /* The first line, inc. newline */
            (1 + indent + width) * approxLines *
                3; /* Justfied lines, assuming bad line breaks */
...
static RETSIGTYPE unset_signal(void) { signal(SIGINT, old_catcher); }
#else
...
void opt_menu_getline(char *prompt, char *respond, int maxlen) {
#if HAVE_READLINE
  char *line;
...
#define OPT_isflagtype(o)                                                      \
  ((o) == OPT_TOGGLE || (o) == OPT_BOOL || (o) == OPT_NEGTOGGLE ||             \
   (o) == OPT_NEGBOOL)
...
static char *optarray_getstrval(int size, void *val, opt_TYPE otype,
                                char delim) {
  int n;
  int len = 0;
...
int optrega_array(int *size, void *val, opt_TYPE otype, char name,
...
int optregp_array(int *size, void *val, opt_TYPE otype, char *longname,
                  char *descript) {
  int n = optrega_array(size, val, otype, '\0', longname, descript);
  optmode_n(n, OPT_POSITIONAL);
  return n;
}
int optregf_array(int *size, void *val, opt_TYPE otype, char name,
                  char *longname, char *descript) {
...
int optregf(void *val, opt_TYPE otype, char name, char *longname,
            char *descript) {
...
int optrega(void *val, opt_TYPE otype, char name, char *longname,
            char *descript) {
...
void optmode_n(int n, opt_MODE mode) {
  if (OPT_isvalidnum(n)) {
    if (!(mode == OPT_POSITIONAL || mode == OPT_DELIMITED ||
          mode == OPT_FLEXIBLE))
...
void optmode(void *v, opt_MODE mode) { optmode_n(opt_number(v), mode); }
...
void optExitNumber(int n) { opt_exit_number = n; }
...
static int opt_longname_number(char *s) {
  int i; /* see which registered option */
  if (ISEMPTYSTRING(s))
    return -1;
  for (i = 0; i < opt_nreg; ++i) {
...
static char *opt_getstrval(void *val, opt_TYPE typ) {
  ...
  case OPT_BOOL:
    ...
    opt_snprintf_1(stval_buf, 80, "%c",
                   (maybe == OPT_TRUE ? FLAG_TRUE : FLAG_FALSE));
    break;
  case OPT_NEGTOGGLE:
  case OPT_NEGBOOL:
    maybe = OPT_GETVALUE(int, val);
    opt_snprintf_1(stval_buf, 80, "%c",
                   (maybe == OPT_TRUE ? FLAG_FALSE : FLAG_TRUE));
    break;
  case OPT_CSTRING:
  ...
  case OPT_STRING:
    if (val == NULL) /* this should never happen */
      sprintf(stval_buf, "\"\"");
    else if (OPT_GETVALUE(char *, val) == NULL)
      sprintf(stval_buf, "\"\"");
    else
...
static int optsizeof(opt_TYPE typ) {
  int retval = 0;
...
char *optstrtyp(int i) {
  ...
  switch (o) {
    /* Integers */
  case OPT_INT:
    strcpy(sttyp_buf, "<INT");
    break;
  case OPT_UINT:
    strcpy(sttyp_buf, "<UINT");
    break;
  case OPT_SHORT:
    strcpy(sttyp_buf, "<SHORT");
    break;
  case OPT_LONG:
    strcpy(sttyp_buf, "<LONG");
    break;
  case OPT_USHORT:
    strcpy(sttyp_buf, "<USHORT");
    break;
  case OPT_ULONG:
    strcpy(sttyp_buf, "<ULONG");
    break;
    /* Floating point */
  case OPT_FLOAT:
    strcpy(sttyp_buf, "<FLOAT");
    break;
  case OPT_DOUBLE:
    strcpy(sttyp_buf, "<DOUBLE");
    break;
    /* Char */
  case OPT_CHAR:
    strcpy(sttyp_buf, "<CHAR");
    break;
  case OPT_UCHAR:
    strcpy(sttyp_buf, "<UCHAR");
    break;
    /* Flag */
  case OPT_INTLEVEL:
    strcpy(sttyp_buf, "<INTLEVEL");
    break;
  case OPT_TOGGLE:
    strcpy(sttyp_buf, "<TOGGLE");
    break;
  case OPT_BOOL:
    strcpy(sttyp_buf, "<BOOL");
    break;
  case OPT_NEGTOGGLE:
    strcpy(sttyp_buf, "<NEGTOGGLE");
    break;
  case OPT_NEGBOOL:
    strcpy(sttyp_buf, "<NEGBOOL");
    break;
    /* String */
  case OPT_STRING:
...
void opt_tofile(FILE *fp) {
  ...
  for (i = 0; i < opt_nreg; ++i) {
    ...
    if (OPT_isflagtype(optlist[i].type) || optlist[i].type == OPT_INTLEVEL ||
...
static void opt_fileheader(FILE *fp) {
  extern char *opt_program_name;
  int i, n;
  char buf[80];
...
char *opt_mstring(int i) {
  ...
  if (optlist[i].name)
    sprintf(mstring, format, optlist[i].name, s, 35 - strlen(s) - 3, "",
            justified_descript);
  else if (optlist[i].longname)
    sprintf(mstring, lformat, optlist[i].longname, s,
            35 - strlen(optlist[i].longname) - strlen(s) - 3, "",
            justified_descript);
...
void opt_usage(void) {
  ...
  for (i = 0; i < opt_nreg; ++i) {
    if (optlist[i].mode != OPT_DELIMITED) {
      ...
      justified_descript = opt_justify(optlist[i].descript, OPTDESCRIPTWIDTH,
                                       OPTDESCRIPTOFFSET, 0, 0);
      if (OPTUSAGEDEBUG)
        fprintf(stderr, "%2d u ", i);
      if (!ISEMPTYSTRING(optlist[i].descript))
        opt_mess_3(pformat, optlist[i].longname, optstrtyp(i),
                   justified_descript);
      else
  ...
  for (i = 0; i < opt_nreg; ++i) {
    if (optlist[i].mode != OPT_POSITIONAL &&
      ...
      if (optlist[i].mode == OPT_FLEXIBLE) {
        justified_descript =
            opt_justify("(See argument description above)", OPTDESCRIPTWIDTH,
                        OPTDESCRIPTOFFSET, 0, 0);
      } else if (optlist[i].longname &&
                 strlen(optlist[i].longname) > OPTMAXLONGNAMELEN) {
        justified_descript =
            opt_justify(optlist[i].descript, OPTDESCRIPTWIDTH,
                        OPTDESCRIPTOFFSET, OPTDESCRIPTOFFSET, 0);
      } else {
        justified_descript = opt_justify(optlist[i].descript, OPTDESCRIPTWIDTH,
                                         OPTDESCRIPTOFFSET, 0, 0);
      }
      ...
      if (optlist[i].name == '\0') {
        ...
        if (!ISEMPTYSTRING(optlist[i].longname)) {
          ...
          if (OPTUSAGEDEBUG)
            fprintf(stderr, "d ");
          opt_mess_3(dformat, optlist[i].name, optstrtyp(i),
                     justified_descript);
        }
      }
      free(justified_descript);
...
int opt_parse_positional(ARGVECTOR *ag) {
  int i;
...
int opt_parse_delim(ARGVECTOR *ag) {
  int i;
  char c;
...
int opt_parse_longdelim(ARGVECTOR *ag) {
  int i;
  char c;
  char *sc, *scptr;
...
static void optarray_action(OptArray_t *arr, char *stmp) {
  ...
  *(arr->size) = 0;
  ...
  while (*t) {
    ...
    *(arr->size) += 1;
    *(arr->value) =
        realloc(*(arr->value), *(arr->size) * optsizeof(arr->base_type));
    /* v points to the next element in the array */
    v = (char *)(*(arr->value)) +
        (*(arr->size) - 1) * optsizeof(arr->base_type);
...
static int opt_action(int i, ARGVECTOR *ag) {
  /* ------------- */
  /* act on option */
  /* ------------- */
  opt_TYPE o;
...
static void opt_xml(int i, FILE *fp) {
  ...
  fprintf(fp, ">\n");
  if (optlist[i].name)
    fprintf(fp, "    <short>%c</short>\n", optlist[i].name);
  if (optlist[i].longname)
    fprintf(fp, "    <long>%s</long>\n", optlist[i].longname);
  if (optlist[i].descript)
...
#include "opt.h"
int optrega_CHAR(char *v, char c, char *n, char *B) {
  return optrega(v, OPT_CHAR, c, n, B);
}
int optreg_CHAR(char *v, char c, char *B) { return optreg(v, OPT_CHAR, c, B); }
int optregc_CHAR(char *v, char c) { return optregc(v, OPT_CHAR, c); }
int optregs_CHAR(char *v, char *n) { return optregs(v, OPT_CHAR, n); }
int optregcb_CHAR(char *v, char c, char *B) {
  return optregcb(v, OPT_CHAR, c, B);
}
int optregsb_CHAR(char *v, char *s, char *B) {
  return optregsb(v, OPT_CHAR, s, B);
}
int optrega_CSTRING(char *v, char c, char *n, char *B) {
  return optrega(v, OPT_CSTRING, c, n, B);
}
int optreg_CSTRING(char *v, char c, char *B) {
  return optreg(v, OPT_CSTRING, c, B);
}
int optregc_CSTRING(char *v, char c) { return optregc(v, OPT_CSTRING, c); }
int optregs_CSTRING(char *v, char *n) { return optregs(v, OPT_CSTRING, n); }
int optregcb_CSTRING(char *v, char c, char *B) {
  return optregcb(v, OPT_CSTRING, c, B);
}
int optregsb_CSTRING(char *v, char *s, char *B) {
  return optregsb(v, OPT_CSTRING, s, B);
}
int optrega_UNDELIM(char **v, char c, char *n, char *B) {
  return optrega(v, OPT_UNDELIM, c, n, B);
}
int optreg_UNDELIM(char **v, char c, char *B) {
  return optreg(v, OPT_UNDELIM, c, B);
}
int optregc_UNDELIM(char **v, char c) { return optregc(v, OPT_UNDELIM, c); }
int optregs_UNDELIM(char **v, char *n) { return optregs(v, OPT_UNDELIM, n); }
int optregcb_UNDELIM(char **v, char c, char *B) {
  return optregcb(v, OPT_UNDELIM, c, B);
}
int optregsb_UNDELIM(char **v, char *s, char *B) {
  return optregsb(v, OPT_UNDELIM, s, B);
}
int optrega_VSTRING(char **v, char c, char *n, char *B) {
  return optrega(v, OPT_VSTRING, c, n, B);
}
int optreg_VSTRING(char **v, char c, char *B) {
  return optreg(v, OPT_VSTRING, c, B);
}
int optregc_VSTRING(char **v, char c) { return optregc(v, OPT_VSTRING, c); }
int optregs_VSTRING(char **v, char *n) { return optregs(v, OPT_VSTRING, n); }
int optregcb_VSTRING(char **v, char c, char *B) {
  return optregcb(v, OPT_VSTRING, c, B);
}
int optregsb_VSTRING(char **v, char *s, char *B) {
  return optregsb(v, OPT_VSTRING, s, B);
}
int optrega_DOUBLE(double *v, char c, char *n, char *B) {
  return optrega(v, OPT_DOUBLE, c, n, B);
}
int optreg_DOUBLE(double *v, char c, char *B) {
  return optreg(v, OPT_DOUBLE, c, B);
}
int optregc_DOUBLE(double *v, char c) { return optregc(v, OPT_DOUBLE, c); }
int optregs_DOUBLE(double *v, char *n) { return optregs(v, OPT_DOUBLE, n); }
int optregcb_DOUBLE(double *v, char c, char *B) {
  return optregcb(v, OPT_DOUBLE, c, B);
}
int optregsb_DOUBLE(double *v, char *s, char *B) {
  return optregsb(v, OPT_DOUBLE, s, B);
}
int optrega_NUL(void *v, char c, char *n, char *B) {
  return optrega(v, OPT_NUL, c, n, B);
}
int optreg_NUL(void *v, char c, char *B) { return optreg(v, OPT_NUL, c, B); }
int optregc_NUL(void *v, char c) { return optregc(v, OPT_NUL, c); }
int optregs_NUL(void *v, char *n) { return optregs(v, OPT_NUL, n); }
int optregcb_NUL(void *v, char c, char *B) {
  return optregcb(v, OPT_NUL, c, B);
}
int optregsb_NUL(void *v, char *s, char *B) {
  return optregsb(v, OPT_NUL, s, B);
}
int optrega_BOOL(int *v, char c, char *n, char *B) {
  return optrega(v, OPT_BOOL, c, n, B);
}
int optreg_BOOL(int *v, char c, char *B) { return optreg(v, OPT_BOOL, c, B); }
int optregc_BOOL(int *v, char c) { return optregc(v, OPT_BOOL, c); }
int optregs_BOOL(int *v, char *n) { return optregs(v, OPT_BOOL, n); }
int optregcb_BOOL(int *v, char c, char *B) {
  return optregcb(v, OPT_BOOL, c, B);
}
int optregsb_BOOL(int *v, char *s, char *B) {
  return optregsb(v, OPT_BOOL, s, B);
}
int optrega_UNDELIMV(char **v, char c, char *n, char *B) {
  return optrega(v, OPT_UNDELIMV, c, n, B);
}
int optreg_UNDELIMV(char **v, char c, char *B) {
  return optreg(v, OPT_UNDELIMV, c, B);
}
int optregc_UNDELIMV(char **v, char c) { return optregc(v, OPT_UNDELIMV, c); }
int optregs_UNDELIMV(char **v, char *n) { return optregs(v, OPT_UNDELIMV, n); }
int optregcb_UNDELIMV(char **v, char c, char *B) {
  return optregcb(v, OPT_UNDELIMV, c, B);
}
int optregsb_UNDELIMV(char **v, char *s, char *B) {
  return optregsb(v, OPT_UNDELIMV, s, B);
}
int optrega_LONG(long *v, char c, char *n, char *B) {
  return optrega(v, OPT_LONG, c, n, B);
}
int optreg_LONG(long *v, char c, char *B) { return optreg(v, OPT_LONG, c, B); }
int optregc_LONG(long *v, char c) { return optregc(v, OPT_LONG, c); }
int optregs_LONG(long *v, char *n) { return optregs(v, OPT_LONG, n); }
int optregcb_LONG(long *v, char c, char *B) {
  return optregcb(v, OPT_LONG, c, B);
}
int optregsb_LONG(long *v, char *s, char *B) {
  return optregsb(v, OPT_LONG, s, B);
}
int optrega_UINT(unsigned int *v, char c, char *n, char *B) {
  return optrega(v, OPT_UINT, c, n, B);
}
int optreg_UINT(unsigned int *v, char c, char *B) {
  return optreg(v, OPT_UINT, c, B);
}
int optregc_UINT(unsigned int *v, char c) { return optregc(v, OPT_UINT, c); }
int optregs_UINT(unsigned int *v, char *n) { return optregs(v, OPT_UINT, n); }
int optregcb_UINT(unsigned int *v, char c, char *B) {
  return optregcb(v, OPT_UINT, c, B);
}
int optregsb_UINT(unsigned int *v, char *s, char *B) {
  return optregsb(v, OPT_UINT, s, B);
}
int optrega_UNSINT(unsigned int *v, char c, char *n, char *B) {
  return optrega(v, OPT_UNSINT, c, n, B);
}
int optreg_UNSINT(unsigned int *v, char c, char *B) {
  return optreg(v, OPT_UNSINT, c, B);
}
int optregc_UNSINT(unsigned int *v, char c) {
  return optregc(v, OPT_UNSINT, c);
}
int optregs_UNSINT(unsigned int *v, char *n) {
  return optregs(v, OPT_UNSINT, n);
}
int optregcb_UNSINT(unsigned int *v, char c, char *B) {
  return optregcb(v, OPT_UNSINT, c, B);
}
int optregsb_UNSINT(unsigned int *v, char *s, char *B) {
  return optregsb(v, OPT_UNSINT, s, B);
}
int optrega_STRING(char **v, char c, char *n, char *B) {
  return optrega(v, OPT_STRING, c, n, B);
}
int optreg_STRING(char **v, char c, char *B) {
  return optreg(v, OPT_STRING, c, B);
}
int optregc_STRING(char **v, char c) { return optregc(v, OPT_STRING, c); }
int optregs_STRING(char **v, char *n) { return optregs(v, OPT_STRING, n); }
int optregcb_STRING(char **v, char c, char *B) {
  return optregcb(v, OPT_STRING, c, B);
}
int optregsb_STRING(char **v, char *s, char *B) {
  return optregsb(v, OPT_STRING, s, B);
}
int optrega_UCHAR(unsigned char *v, char c, char *n, char *B) {
  return optrega(v, OPT_UCHAR, c, n, B);
}
int optreg_UCHAR(unsigned char *v, char c, char *B) {
  return optreg(v, OPT_UCHAR, c, B);
}
int optregc_UCHAR(unsigned char *v, char c) { return optregc(v, OPT_UCHAR, c); }
int optregs_UCHAR(unsigned char *v, char *n) {
  return optregs(v, OPT_UCHAR, n);
}
int optregcb_UCHAR(unsigned char *v, char c, char *B) {
  return optregcb(v, OPT_UCHAR, c, B);
}
int optregsb_UCHAR(unsigned char *v, char *s, char *B) {
  return optregsb(v, OPT_UCHAR, s, B);
}
int optrega_SHORT(short *v, char c, char *n, char *B) {
  return optrega(v, OPT_SHORT, c, n, B);
}
int optreg_SHORT(short *v, char c, char *B) {
  return optreg(v, OPT_SHORT, c, B);
}
int optregc_SHORT(short *v, char c) { return optregc(v, OPT_SHORT, c); }
int optregs_SHORT(short *v, char *n) { return optregs(v, OPT_SHORT, n); }
int optregcb_SHORT(short *v, char c, char *B) {
  return optregcb(v, OPT_SHORT, c, B);
}
int optregsb_SHORT(short *v, char *s, char *B) {
  return optregsb(v, OPT_SHORT, s, B);
}
int optrega_USHORT(unsigned short *v, char c, char *n, char *B) {
  return optrega(v, OPT_USHORT, c, n, B);
}
int optreg_USHORT(unsigned short *v, char c, char *B) {
  return optreg(v, OPT_USHORT, c, B);
}
int optregc_USHORT(unsigned short *v, char c) {
  return optregc(v, OPT_USHORT, c);
}
int optregs_USHORT(unsigned short *v, char *n) {
  return optregs(v, OPT_USHORT, n);
}
int optregcb_USHORT(unsigned short *v, char c, char *B) {
  return optregcb(v, OPT_USHORT, c, B);
}
int optregsb_USHORT(unsigned short *v, char *s, char *B) {
  return optregsb(v, OPT_USHORT, s, B);
}
int optrega_INT(int *v, char c, char *n, char *B) {
  return optrega(v, OPT_INT, c, n, B);
}
int optreg_INT(int *v, char c, char *B) { return optreg(v, OPT_INT, c, B); }
int optregc_INT(int *v, char c) { return optregc(v, OPT_INT, c); }
int optregs_INT(int *v, char *n) { return optregs(v, OPT_INT, n); }
int optregcb_INT(int *v, char c, char *B) { return optregcb(v, OPT_INT, c, B); }
int optregsb_INT(int *v, char *s, char *B) {
  return optregsb(v, OPT_INT, s, B);
}
int optrega_NEGBOOL(int *v, char c, char *n, char *B) {
  return optrega(v, OPT_NEGBOOL, c, n, B);
}
int optreg_NEGBOOL(int *v, char c, char *B) {
  return optreg(v, OPT_NEGBOOL, c, B);
}
int optregc_NEGBOOL(int *v, char c) { return optregc(v, OPT_NEGBOOL, c); }
int optregs_NEGBOOL(int *v, char *n) { return optregs(v, OPT_NEGBOOL, n); }
int optregcb_NEGBOOL(int *v, char c, char *B) {
  return optregcb(v, OPT_NEGBOOL, c, B);
}
int optregsb_NEGBOOL(int *v, char *s, char *B) {
  return optregsb(v, OPT_NEGBOOL, s, B);
}
int optrega_UNDELIMC(char *v, char c, char *n, char *B) {
  return optrega(v, OPT_UNDELIMC, c, n, B);
}
int optreg_UNDELIMC(char *v, char c, char *B) {
  return optreg(v, OPT_UNDELIMC, c, B);
}
int optregc_UNDELIMC(char *v, char c) { return optregc(v, OPT_UNDELIMC, c); }
int optregs_UNDELIMC(char *v, char *n) { return optregs(v, OPT_UNDELIMC, n); }
int optregcb_UNDELIMC(char *v, char c, char *B) {
  return optregcb(v, OPT_UNDELIMC, c, B);
}
int optregsb_UNDELIMC(char *v, char *s, char *B) {
  return optregsb(v, OPT_UNDELIMC, s, B);
}
int optrega_ULONG(unsigned long *v, char c, char *n, char *B) {
  return optrega(v, OPT_ULONG, c, n, B);
}
int optreg_ULONG(unsigned long *v, char c, char *B) {
  return optreg(v, OPT_ULONG, c, B);
}
int optregc_ULONG(unsigned long *v, char c) { return optregc(v, OPT_ULONG, c); }
int optregs_ULONG(unsigned long *v, char *n) {
  return optregs(v, OPT_ULONG, n);
}
int optregcb_ULONG(unsigned long *v, char c, char *B) {
  return optregcb(v, OPT_ULONG, c, B);
}
int optregsb_ULONG(unsigned long *v, char *s, char *B) {
  return optregsb(v, OPT_ULONG, s, B);
}
int optrega_FLOAT(float *v, char c, char *n, char *B) {
  return optrega(v, OPT_FLOAT, c, n, B);
}
int optreg_FLOAT(float *v, char c, char *B) {
  return optreg(v, OPT_FLOAT, c, B);
}
int optregc_FLOAT(float *v, char c) { return optregc(v, OPT_FLOAT, c); }
int optregs_FLOAT(float *v, char *n) { return optregs(v, OPT_FLOAT, n); }
int optregcb_FLOAT(float *v, char c, char *B) {
  return optregcb(v, OPT_FLOAT, c, B);
}
int optregsb_FLOAT(float *v, char *s, char *B) {
  return optregsb(v, OPT_FLOAT, s, B);
}
int optrega_NEGTOGGLE(int *v, char c, char *n, char *B) {
  return optrega(v, OPT_NEGTOGGLE, c, n, B);
}
int optreg_NEGTOGGLE(int *v, char c, char *B) {
  return optreg(v, OPT_NEGTOGGLE, c, B);
}
int optregc_NEGTOGGLE(int *v, char c) { return optregc(v, OPT_NEGTOGGLE, c); }
int optregs_NEGTOGGLE(int *v, char *n) { return optregs(v, OPT_NEGTOGGLE, n); }
int optregcb_NEGTOGGLE(int *v, char c, char *B) {
  return optregcb(v, OPT_NEGTOGGLE, c, B);
}
int optregsb_NEGTOGGLE(int *v, char *s, char *B) {
  return optregsb(v, OPT_NEGTOGGLE, s, B);
}
int optrega_INTLEVEL(int *v, char c, char *n, char *B) {
  return optrega(v, OPT_INTLEVEL, c, n, B);
}
int optreg_INTLEVEL(int *v, char c, char *B) {
  return optreg(v, OPT_INTLEVEL, c, B);
}
int optregc_INTLEVEL(int *v, char c) { return optregc(v, OPT_INTLEVEL, c); }
int optregs_INTLEVEL(int *v, char *n) { return optregs(v, OPT_INTLEVEL, n); }
int optregcb_INTLEVEL(int *v, char c, char *B) {
  return optregcb(v, OPT_INTLEVEL, c, B);
}
int optregsb_INTLEVEL(int *v, char *s, char *B) {
  return optregsb(v, OPT_INTLEVEL, s, B);
}
int optrega_TOGGLE(int *v, char c, char *n, char *B) {
  return optrega(v, OPT_TOGGLE, c, n, B);
}
int optreg_TOGGLE(int *v, char c, char *B) {
  return optreg(v, OPT_TOGGLE, c, B);
}
int optregc_TOGGLE(int *v, char c) { return optregc(v, OPT_TOGGLE, c); }
int optregs_TOGGLE(int *v, char *n) { return optregs(v, OPT_TOGGLE, n); }
int optregcb_TOGGLE(int *v, char c, char *B) {
  return optregcb(v, OPT_TOGGLE, c, B);
}
int optregsb_TOGGLE(int *v, char *s, char *B) {
 ...
 * set title, usage, etc.  *
 ***************************/
void optTitle(char *s) { opt_setstring(&opt_titlestring, s); }
...
void optUsage(char *s) { opt_setstring(&opt_usagestring, s); }
void optEnvVarName(char *s) { opt_setstring(&opt_envstring, s); }
void optDefaultString(char *s) { opt_setstring(&opt_defstring, s); }
void optDefaultFile(char *s) { opt_setstring(&opt_defaultfile, s); }
void optQuit(OPT_PFI fcn) { opt_quit_fcn = fcn; }
void optRun(OPT_PFI fcn) { opt_run_fcn = fcn; }
void optMain(OPT_PFI_ARG fcn) { opt_main_fcn = fcn; }
void optAdditionalUsage(OPT_PFI fcn) { opt_additional_usage_fcn = fcn; }
void optDisableMenu() { opt_menu_enabled = OPT_FALSE; }
 ...
 */
void opt(int *argcptr, char ***argvptr) {
  int argc;
  char **argv;
  ARGVECTOR *ag;
...
ARGVECTOR *opt_process(int argc, char **argv) {
  char c;
  int iword;
  ARGVECTOR *ag;
    ...
    case RUN:
      ...
      case BANG:
        ...
        if (!opt_menuflag)
          ...
          opt_warn_1("Shell to system invalid unless \'%c\' is the first "
...
static int line2argv(int *pargc, char ***pargv, char *line) {
  char *lincp;
  if (line == NULL) {
    *pargc = 0;
    **pargv = NULL;
...
static int l2a(char *line, char **argv) {
...
static int break_word(int inquote, char *line) {
...
int opt_getline(char *line, FILE *fp) {
  int c;
  int count = 0;
...
ARGVECTOR *ag_new(int argc, char **argv) {
  ARGVECTOR *ag;
...
void ag_free(ARGVECTOR *ag) { free((char *)ag); }
void ag_fprint(FILE *fp, ARGVECTOR *ag) {
  int i;
  if (ag == NULL) {
    fprintf(fp, "ag=<nul>\n");
    return;
...
void ag_reset(ARGVECTOR *ag) {
  ag->iw = 0;
  ag->ic = 0;
}
int ag_w_number(ARGVECTOR *ag) { return ag->iw; }
void ag_w_advance(ARGVECTOR *ag) {
...
/*      ag_c:   return current character
                do not advance
*/
char ag_c(ARGVECTOR *ag) { return (ag->v[ag->iw][ag->ic]); }
char ag_c_next(ARGVECTOR *ag) { return (ag->v[ag->iw][ag->ic + 1]); }
char ag_cnn_next(ARGVECTOR *ag) {
  if (ag_c_next(ag) == '\0') {
    if (ag->iw + 1 >= ag->c)
      return ('\0');
    else
...
*/
char ag_c_advance(ARGVECTOR *ag) {
...
void ag_backword(ARGVECTOR *ag) { ag->iw -= 1; }

char ag_backspace(ARGVECTOR *ag) {
...
*/
char *ag_s(ARGVECTOR *ag) {
  if (ag->iw < ag->c)
    return (ag->v[ag->iw] + ag->ic);
  else
    return (NULL);
}
char *ag_s_next(ARGVECTOR *ag) {
...
*/
char *ag_s_advance(ARGVECTOR *ag) {
...
int ag_clear(ARGVECTOR *ag) {
  while (!ag_end(ag))
    argnext(ag);
  return 1;
}

int ag_argc(ARGVECTOR *ag) { return ag->c - ag->iw; }
char **ag_argv(ARGVECTOR *ag) { return ag->v + ag->iw; }
ARGVECTOR *ag_copy(ARGVECTOR *ag) {
...
ARGVECTOR *ag_prepend_argv0(ARGVECTOR *ag, char *argv0) {
  int i;
  ARGVECTOR *agnew;
  agnew = (ARGVECTOR *)malloc(sizeof(ARGVECTOR));
...
int OptRegister(unsigned short *v, opt_TYPE o, opt_MODE m, char c, char *n,
                char *B) {
  int i = optrega(v, o, c, n, B);
  optmode_n(i, m);
  return i;
...
int OptRegister(unsigned long *v, opt_TYPE o, opt_MODE m, char c, char *n,
                char *B) {
  int i = optrega(v, o, c, n, B);
  optmode_n(i, m);
  return i;
...
int OptRegister(unsigned int *v, opt_TYPE o, opt_MODE m, char c, char *n,
                char *B) {
  int i = optrega(v, o, c, n, B);
  optmode_n(i, m);
  return i;
...
int OptRegister(unsigned char *v, opt_TYPE o, opt_MODE m, char c, char *n,
                char *B) {
  int i = optrega(v, o, c, n, B);
  optmode_n(i, m);
  return i;
...
int opt_isvalidnumber(char *t) {
  int status = OPT_NUM_VALID; /* innocent until proven guilty */

  if (!t)
    return FALSE; /* null string is not a number */
  ...
  if (*t == 'e' || *t == 'E') {
    ++t;
    if (*t == '\0')
      return FALSE; /* '5e' isn't a number! */
    if (*t != '\0' && IsSign(*t)) {
      if (*t == '-')
        status |= OPT_NUM_FLOAT; /* '1e-3' = 0.001 is FLOAT */
      ++t;
    }
    while (*t != '\0' && IsDigit(*t))
      ++t;
...
unsigned long opt_atou(char *s) {
  int valid;
  unsigned long x;
  x = (unsigned long)atof(s);
  valid = opt_isvalidnumber(s);
  if (!valid || (valid & OPT_NUM_FLOAT) || (valid & OPT_NUM_NEG)) {
    opt_warn_2("String [%s] is not a valid positive integer, will use [%lu]", s,
               x);
  }
  return x;
}
